...
# Hàm chọn kết quả tối ưu nhất
def select_optimal_result(normalized_scores):
    optimal_results = {}
    for method, scores in normalized_scores.items():
        optimal_results[method] = max(scores)
    optimal_method = max(optimal_results, key=optimal_results.get)
    return optimal_method, optimal_results[optimal_method]

# Hàm chọn chuỗi tương đồng nhất hoặc None
def select_most_similar_string(normalized_scores, corpus, threshold=0.1):
    combined_scores = np.mean(list(normalized_scores.values()), axis=0)
    top_scores_indices = np.argsort(combined_scores)[::-1][:10]
    top_scores = combined_scores[top_scores_indices]

    if len(top_scores) < 2 or (top_scores[0] - top_scores[1]) > threshold:
        return corpus[top_scores_indices[0]]
    return None
...
def fuzzy_search(query, corpus):
    ...
    # Chọn kết quả tối ưu nhất
    optimal_method, optimal_score = select_optimal_result(results)
    
    # Chọn chuỗi tương đồng nhất hoặc None
    most_similar_string = select_most_similar_string(results, corpus)
    
    return results, optimal_method, optimal_score, most_similar_string
...
query = "programming"
# Kết quả tìm kiếm
results, optimal_method, optimal_score, most_similar_string = fuzzy_search(query, corpus)

# In kết quả
for method, result in results.items():
    print(f"{method}: {result}")
print(f"Optimal Method: {optimal_method} with score {optimal_score}")
print(f"Most Similar String: {most_similar_string}")
